...
# Fonction pour récupérer les genres d'un artiste
def get_artist_genres(artist_id):
    try:
        artist = spotify.artist(artist_id)
        return artist['genres']
    except Exception as e:
        print(f"Erreur lors de la récupération des genres pour l'artiste {artist_id} : {e}")
        return []
...
def get_playlist_tracks(playlist_id, playlist_name):
    ...
    for i, item in enumerate(items):
        ...
        if track and track['artists']:
            artist_id = track['artists'][0]['id']  # Utilise le premier artiste
        ...
        tracks.append({
            ...
            "Genres musicaux": ', '.join(artist_genres)
        })
    return tracks
...
csv_filename = "morceaux_par_playlists_avec_genres2.csv"
